  ...
  return (
    <section id="service" className="section" data-screen-label="05 Service" style={{background:'var(--bg-parchment)'}}>
      <div className="container">
        <window.Reveal y={20}>
          <div style={{textAlign:'center', maxWidth:760, margin:'0 auto 56px'}}>
            <div className="eyebrow" style={{marginBottom:20, justifyContent:'center', display:'flex'}}>
              <span className="dash"></span>SERVICE / 05<span className="dash"></span>
            </div>
            <h2 className="h-section" style={{marginBottom:20, textWrap:'balance', wordBreak:'auto-phrase'}}>
              業務の階層・規模に合わせて、<br />3つのコース。
            </h2>
            <p className="lead">
        ...
        {/* Tab selector */}
        <div role="tablist" style={{display:'flex', justifyContent:'center', gap:6, marginBottom:32, flexWrap:'wrap'}}>
          {courses.map((co, i) => (
            <button key={co.tag} role="tab" aria-selected={tab===i} onClick={()=>setTab(i)}
              style={{
              ...
              <span style={{opacity:0.7, marginRight:8, fontSize:11}}>0{i+1}</span>{co.tag}コース
            </button>
          ))}
        </div>
        ...
        {/* Course detail */}
        <div key={tab} style={{
          background:'var(--bg-ivory)', border:'1px solid var(--border-1)',
          borderRadius:18, padding:'clamp(28px, 4vw, 48px)',
          display:'grid', gridTemplateColumns:'minmax(0, 1fr) minmax(0, 1.1fr)', gap:'clamp(28px, 4vw, 56px)',
          animation: 'slidein 0.5s cubic-bezier(0.22,0.61,0.36,1) both',
        }}>
          <div>
            <div style={{display:'inline-flex', alignItems:'center', gap:8, padding:'4px 14px', borderRadius:9999, background:c.featured?'var(--brand)':'var(--brand-soft)', color:c.featured?'#fff':'var(--brand-dark)', fontSize:11, fontWeight:500, letterSpacing:0.4, marginBottom:18}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, paddingTop:20, borderTop:'1px solid var(--border-1)'}}>
              {[['対象', c.target], ['期間', c.duration], ['料金', c.price], ['実施形式', 'オンライン / 訪問']].map(([k,v])=> (
                <div key={k}>
                  <div style={{fontSize:11, color:'var(--ink-3)', letterSpacing:0.5, marginBottom:4}}>{k}</div>
          ...
          <div>
            <div style={{fontSize:11, color:'var(--ink-3)', letterSpacing:0.5, marginBottom:14, textTransform:'uppercase'}}>カリキュラム例</div>
            <div style={{display:'flex', flexDirection:'column'}}>
              {c.curriculum.map(([t, item], i) => (
                <div key={i} style={{display:'grid', gridTemplateColumns:'90px 1fr', gap:14, padding:'12px 0', borderTop: i===0 ? 'none' : '1px solid var(--border-1)'}}>
                  <div style={{fontFamily:'var(--serif)', fontSize:13, color:'var(--brand)', fontWeight:500}}>{t}</div>
        ...
        <div style={{marginTop:32, display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))', gap:12}}>
          {[
            ...
            <div key={h} style={{background:'var(--bg-white)', border:'1px solid var(--border-2)', borderRadius:12, padding:'18px 20px'}}>
              <div style={{fontFamily:'var(--serif)', fontSize:15, fontWeight:500, color:'var(--ink-1)', marginBottom:6}}>{h}</div>
...
function Strengths() {
  ...
  return (
    <section id="strengths" className="section" data-screen-label="06 Strengths" style={{background:'var(--bg-ivory)'}}>
      <div className="container">
        <window.Reveal y={20}>
          <div style={{display:'grid', gridTemplateColumns:'minmax(0, 1fr) minmax(0, 1fr)', gap:'clamp(28px, 5vw, 80px)', alignItems:'end', marginBottom:'clamp(40px, 5vw, 64px)'}}>
            <div>
              <div className="eyebrow" style={{marginBottom:20}}>
                <span className="dash"></span>WHY US / 06
              </div>
              <h2 className="h-section" style={{textWrap:'balance', wordBreak:'auto-phrase'}}>
                「業務が変わる」<span className="accent-color">6つの</span>理由。
              </h2>
            </div>
        ...
        <window.Stagger step={0.06} y={20} className="strengths-grid">
          {strengths.map(({num, title, desc}, i) => (
            <div key={num} className="strength-cell">
              ...
              <p style={{fontSize:14, color:'var(--ink-2)', lineHeight:1.85, textWrap:'pretty', wordBreak:'auto-phrase'}}>{desc}</p>
            </div>
          ))}
        </window.Stagger>
      </div>
  ...
  return (
    <section id="results" className="section section--dark" data-screen-label="07 Results">
      <div className="container">
        <window.Reveal y={20}>
          <div style={{textAlign:'center', maxWidth:760, margin:'0 auto 56px'}}>
            <div className="eyebrow on-dark" style={{marginBottom:20, justifyContent:'center', display:'flex'}}>
              <span className="dash"></span>RESULTS / 07<span className="dash"></span>
            </div>
            <h2 className="h-section" style={{color:'var(--on-dark-1)', marginBottom:18, textWrap:'balance', wordBreak:'auto-phrase'}}>
              「AIを使える」ではなく、<br /><span style={{color:'var(--brand-coral)'}}>業務が変わった</span>企業たち。
            </h2>
        ...
        <window.Reveal delay={0.1} y={28}>
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:0, border:'1px solid var(--border-dark)', borderRadius:16, overflow:'hidden', background:'var(--bg-dark-3)', marginBottom:64}}>
            {[
              ...
              <div key={s.k} style={{padding:'28px 24px', borderRight: i<4 ? '1px solid var(--border-dark)' : 'none'}}>
                <div style={{fontFamily:'var(--serif)', fontSize:'clamp(24px, 3vw, 36px)', fontWeight:500, color:'var(--on-dark-1)', lineHeight:1.05}}>
                  <window.Counter to={s.v} decimals={s.dec || 0} duration={1500} prefix={s.prefix || ''} suffix={s.suffix} />
                </div>
                <div style={{fontSize:12, color:'var(--on-dark-3)', marginTop:8, letterSpacing:0.4}}>{s.k}</div>
              </div>
            ))}
          </div>
        </window.Reveal>
        ...
        <window.Reveal delay={0.18} y={16}>
          <div style={{marginBottom:64}}>
            <div style={{textAlign:'center', fontSize:11, letterSpacing:1.4, color:'var(--on-dark-3)', textTransform:'uppercase', marginBottom:24}}>導入企業（一例）</div>
            <div className="marquee-mask" style={{overflow:'hidden', border:'1px solid var(--border-dark)', borderRadius:14, padding:'24px 0', background:'var(--bg-dark-2)'}}>
              <div className="marquee-track">
                {[...Array(2)].flatMap((_, dup) => (
                  ['Mfg-A工業', '丸〇商事', 'B法律事務所', '△△クリニック', '◇◇設計', '○×サービス', 'Tech-Z株式会社', '　ケア　Care+', 'スタジオ K']
                  .map((l, i) => (
                    <span key={`${dup}-${i}`} style={{fontFamily:'var(--serif)', fontSize:18, color:'var(--on-dark-2)', letterSpacing:1, opacity:0.85, whiteSpace:'nowrap'}}>{l}</span>
                  ))
                ))}
              </div>
            </div>
        ...
        {/* Testimonials */}
        <window.Stagger step={0.1} y={28} style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))', gap:16}}>
          {testimonials.map(t => (
            <div key={t.company} className="card card--dark" style={{display:'flex', flexDirection:'column', gap:0, padding:0, overflow:'hidden'}}>
              {/* Card photo */}
              <div style={{position:'relative', aspectRatio:'16 / 9', overflow:'hidden', background:'var(--bg-dark-3)'}}>
                ...
                <div style={{position:'absolute', left:14, top:14, fontSize:10, color:'var(--brand-coral)', background:'rgba(20,20,19,0.55)', backdropFilter:'blur(8px)', padding:'4px 10px', borderRadius:9999, letterSpacing:0.4}}>
                  {t.industry}
                </div>
              </div>
              <div style={{padding:'20px 22px 22px', display:'flex', flexDirection:'column', gap:14}}>
                <h3 className="h-card" style={{color:'var(--on-dark-1)', fontSize:17, marginBottom:0, textWrap:'balance', wordBreak:'auto-phrase'}}>{t.headline}</h3>
                <p className="body on-dark" style={{fontSize:14, textWrap:'pretty', wordBreak:'auto-phrase'}}>{t.text}</p>

                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, padding:'14px 0', borderTop:'1px solid rgba(255,255,255,0.08)', borderBottom:'1px solid rgba(255,255,255,0.08)'}}>
                  {t.kpi.map(([k, v]) => (
                    <div key={k}>
                      <div style={{fontSize:10, color:'var(--on-dark-3)', letterSpacing:0.4, marginBottom:4}}>{k}</div>
                      <div style={{fontFamily:'var(--serif)', fontSize:18, color:'var(--brand-coral)', fontWeight:500}}>{v}</div>
                    </div>
                  ))}
                </div>

                <div style={{fontSize:12, color:'var(--on-dark-2)'}}>
                  <span style={{color:'var(--on-dark-1)', fontWeight:500}}>{t.company}</span>{' '}/{' '}{t.role}
                </div>
              </div>
            </div>
          ))}
