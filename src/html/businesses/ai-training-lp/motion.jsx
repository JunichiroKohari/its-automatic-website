...
function Stagger({ children, step = 0.08, baseDelay = 0, y = 20, style, ...rest }) {
...
function Counter({ to, decimals = 0, prefix = '', suffix = '', duration = 1400 }) {
  ...
  return (
    <span ref={ref}>{prefix}{Number(val).toLocaleString(undefined, { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}{suffix}</span>
  );
}
...
function Parallax({ children, speed = 0.15, style, ...rest }) {
...
function Bar({ value, max = 100, color = 'var(--brand)', bg = 'var(--border-2)', height = 8, duration = 1100, delay = 0, label, suffix = '' }) {
  const [ref, inView] = useInView({ threshold: 0.4 });
  const pct = Math.min(100, (value / max) * 100);
  return (
    <div ref={ref}>
      {label && (
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', fontSize:12, color:'var(--ink-2)', marginBottom:6}}>
          <span>{label}</span>
...
Object.assign(window, { useInView, Reveal, Stagger, useCountUp, Counter, useParallax, Parallax, useTyping, Bar });
