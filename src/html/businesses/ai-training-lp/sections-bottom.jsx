  ...
  return (
    <section id="flow" className="section" data-screen-label="08 Flow" style={{background:'var(--bg-parchment)'}}>
      <div className="container">
        <window.Reveal y={20}>
          <div style={{maxWidth:880, marginBottom:'clamp(40px, 6vw, 64px)'}}>
            <div className="eyebrow" style={{marginBottom:20}}>
              <span className="dash"></span>PROCESS / 08
            </div>
            <h2 className="h-section" style={{marginBottom:24, textWrap:'balance', wordBreak:'auto-phrase', maxWidth:760}}>
              業務棚卸しから始め、<span className="accent-underline">最短2週間</span>で初回研修。
            </h2>
        ...
        <div style={{display:'grid', gridTemplateColumns:'minmax(0, 1fr)', maxWidth:880, margin:'0 auto'}}>
              ...
              <div key={s.num} style={{
                ...
                <div style={{paddingTop:6, paddingBottom:i === steps.length-1 ? 0 : 8}}>
                  <div style={{display:'flex', alignItems:'baseline', gap:14, flexWrap:'wrap', marginBottom:8}}>
                    ...
                    <span style={{fontSize:12, color:'var(--brand)', background:'var(--brand-soft)', padding:'3px 10px', borderRadius:9999, letterSpacing:0.3, fontWeight:500}}>{s.span}</span>
                  </div>
                  <p className="body" style={{marginBottom:10}}>{s.desc}</p>
                  <div style={{fontSize:13, color:'var(--ink-3)', lineHeight:1.7, paddingLeft:14, borderLeft:'2px solid var(--brand-soft)'}}>
      ...
      <div className="container" style={{maxWidth:920}}>
        <div style={{textAlign:'center', marginBottom:48}}>
          <div className="eyebrow" style={{marginBottom:20, justifyContent:'center', display:'flex'}}>
            <span className="dash"></span>FAQ / 09<span className="dash"></span>
          </div>
        ...
        <div>
          {faqs.map((f, i) => (
            <div key={i} style={{borderTop:'1px solid var(--border-2)'}}>
              ...
              >
                <span style={{color:'var(--brand)', fontFamily:'var(--serif)', fontSize:15, fontWeight:500, flexShrink:0, paddingTop:2, minWidth:24}}>Q{(i+1).toString().padStart(2, '0')}</span>
                <span style={{flex:1, fontSize:'clamp(15px, 1.7vw, 17px)', fontWeight:500, color:'var(--ink-1)', lineHeight:1.55, textWrap:'pretty'}}>{f.q}</span>
                <span style={{
              ...
              {open === i && (
                <div style={{
                  paddingLeft:42, paddingBottom:24, paddingRight:48,
          ...
          <div style={{borderTop:'1px solid var(--border-2)'}}></div>
        </div>

        <div style={{textAlign:'center', marginTop:40, fontSize:14, color:'var(--ink-2)'}}>
          ここに無い質問は、<a href="#contact" style={{color:'var(--brand)', borderBottom:'1px solid currentColor'}}>無料相談</a> にて直接お聞かせください。
        </div>
      </div>
    </section>
  );
    ...
    <section id="contact" className="section section--dark" data-screen-label="10 CTA" style={{position:'relative', overflow:'hidden'}}>
      ...
      <div aria-hidden style={{position:'absolute', inset:0, pointerEvents:'none', zIndex:0}}>
        <img
          ...
          alt=""
          loading="lazy"
          style={{width:'100%', height:'100%', objectFit:'cover', opacity:0.18, filter:'saturate(0.7) contrast(1.05)'}}
        />
        <div style={{position:'absolute', inset:0, background:'linear-gradient(180deg, rgba(20,20,19,0.85) 0%, rgba(20,20,19,0.92) 100%)'}} />
      </div>
      <div aria-hidden style={{position:'absolute', top:-100, right:-100, width:480, height:480, borderRadius:'50%', background:'radial-gradient(circle, rgba(201,100,66,0.18) 0%, transparent 60%)', pointerEvents:'none'}} />
      <div aria-hidden style={{position:'absolute', bottom:-160, left:-120, width:520, height:520, borderRadius:'50%', background:'radial-gradient(circle, rgba(107,127,92,0.10) 0%, transparent 60%)', pointerEvents:'none'}} />

      <div className="container" style={{position:'relative', maxWidth:760, display:'flex', flexDirection:'column', gap:'clamp(32px, 5vw, 56px)'}}>
        <div style={{textAlign:'center'}}>
          <div className="eyebrow on-dark" style={{marginBottom:24, justifyContent:'center', display:'inline-flex'}}>
            <span className="dash"></span>CONTACT / 10<span className="dash"></span>
          </div>
          <h2 className="h-section" style={{color:'var(--on-dark-1)', marginBottom:24, textWrap:'balance', wordBreak:'auto-phrase', maxWidth:680, marginLeft:'auto', marginRight:'auto'}}>
            業務を見直す、<span style={{color:'var(--brand-coral)'}}>最初の一歩</span>を一緒に。
          </h2>
          <p className="lead on-dark" style={{marginBottom:40, textWrap:'pretty', wordBreak:'auto-phrase', maxWidth:620, marginLeft:'auto', marginRight:'auto'}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))', gap:16, marginBottom:36, textAlign:'left'}}>
            {[
              ...
              <div key={h} style={{display:'flex', gap:12, alignItems:'flex-start', padding:'12px 14px', background:'rgba(255,255,255,0.04)', border:'1px solid var(--border-dark)', borderRadius:12}}>
                <span style={{
                ...
                <div>
                  <div style={{fontFamily:'var(--serif)', fontSize:13.5, fontWeight:500, color:'var(--on-dark-1)', marginBottom:3, textWrap:'balance'}}>{h}</div>
        ...
        <div style={{
          ...
          {submitted ? (
            <div style={{padding:'32px 8px', textAlign:'center'}}>
              <div style={{width:56, height:56, borderRadius:'50%', background:'var(--brand-soft)', color:'var(--brand)', margin:'0 auto 20px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:24}}>✓</div>
              <h3 className="h-card" style={{marginBottom:12}}>お問い合わせありがとうございます。</h3>
              <p className="body">担当より1営業日以内にご連絡いたします。お急ぎの方はお電話でも承ります。</p>
            </div>
          ) : (
            <form onSubmit={onSubmit} style={{display:'flex', flexDirection:'column', gap:16}}>
              ...
              <div>
                <label style={{fontSize:12, fontWeight:500, color:'var(--ink-2)', display:'block', marginBottom:8}}>
                  お問い合わせ内容 <span style={{display:'inline-block', marginLeft:6, background:'var(--brand)', color:'#fff', fontSize:10, padding:'1px 6px', borderRadius:3}}>必須</span>
                </label>
              ...
              <div className="field">
                <label>会社メールアドレス <span className="req">必須</span></label>
              ...
              <p style={{fontSize:11, color:'var(--ink-3)', textAlign:'center', lineHeight:1.7, marginTop:4}}>
                ご入力情報は <a href="#" style={{borderBottom:'1px solid currentColor'}}>プライバシーポリシー</a> に従って取り扱います。
              </p>
            </form>
          )}
        </div>
        ...
        {/* Phone fallback */}
        <div style={{padding:'20px 24px', background:'rgba(255,255,255,0.04)', border:'1px solid var(--border-dark)', borderRadius:14, textAlign:'center'}}>
          <div style={{fontSize:11, color:'var(--on-dark-3)', letterSpacing:1, marginBottom:8, textTransform:'uppercase'}}>お電話でのお問い合わせ</div>
          <div style={{fontFamily:'var(--serif)', fontSize:24, fontWeight:500, color:'var(--on-dark-1)', marginBottom:4}}>06-XXXX-XXXX</div>
          <div style={{fontSize:12, color:'var(--on-dark-3)'}}>平日 10:00 - 18:00 / 株式会社It's Automatic</div>
        </div>
      </div>
    </section>
  );
...
function Footer() {
  return (
    <footer style={{background:'var(--bg-dark-2)', color:'var(--on-dark-2)', padding:'56px var(--px) 32px'}}>
      <div className="container">
        <div style={{display:'grid', gridTemplateColumns:'minmax(0, 1.4fr) repeat(3, minmax(0, 1fr))', gap:'clamp(28px, 4vw, 56px)', marginBottom:40}}>
          <div>
            <img src="assets/logo-light.svg" alt="It's Automatic" style={{height:28, width:'auto', marginBottom:16, opacity:0.9}} />
            <p style={{fontSize:13, color:'var(--on-dark-3)', lineHeight:1.85, maxWidth:280}}>
              株式会社It's Automatic<br />
              スマホアプリ・Webサービスの企画・開発・運営。企業向けAI研修を提供しています。
            </p>
            <p style={{fontSize:12, color:'var(--on-dark-3)', marginTop:16, lineHeight:1.7}}>
              〒564-0000 大阪府吹田市<br />
          ...
          {[
            ...
            <div key={h}>
              <div style={{fontSize:11, color:'var(--on-dark-3)', letterSpacing:1.4, textTransform:'uppercase', marginBottom:14}}>{h}</div>
        ...
        <div style={{borderTop:'1px solid var(--border-dark)', paddingTop:24, display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:12, fontSize:12, color:'var(--on-dark-3)'}}>
...
Object.assign(window, { Flow, FAQ, CTA, Footer });
