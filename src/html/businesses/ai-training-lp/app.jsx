...
function SideIndex({ visible }) {
  ...
  return (
    <aside aria-label="LP block index" className="hide-mobile" style={{
      ...
      <div style={{fontSize:9, letterSpacing:1.4, color:'var(--ink-3)', textTransform:'uppercase', textAlign:'center', padding:'4px 4px 8px', borderBottom:'1px solid var(--border-1)', marginBottom:4}}>
        10ブロック
      </div>
      {BLOCKS.map(({ id, no, name }) => (
        <a key={id} href={`#${id}`} style={{
          ...
          <span style={{fontFamily:'var(--serif)', fontSize:11, fontWeight:500, opacity:0.75, minWidth:18}}>{no}</span>
...
function FloatingCTA() {
  ...
  return (
    <>
      {/* Mobile: bottom bar */}
      <div className="mobile-cta show-mobile" style={{
      ...
      >
        ...
        <span style={{display:'flex', flexDirection:'column', alignItems:'flex-start', lineHeight:1.15}}>
          <span style={{fontSize:10, opacity:0.85, letterSpacing:0.6, textTransform:'uppercase'}}>60分 / オンライン</span>
