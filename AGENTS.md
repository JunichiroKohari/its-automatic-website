...
- Install dependencies with `npm ci --ignore-scripts --legacy-peer-deps` in fresh cloud environments.
...
- If a cloud setup still reports `node-sass`, treat it as stale peer-resolution state and keep using `--legacy-peer-deps`.
